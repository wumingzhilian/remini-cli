...
- 已确认决策（`2026-02-27`）：
  ...
  2. Git 允许重写历史提交时间（Author Date / Committer Date），并强推到 `main`
...
4. 允许通过重写历史设置 `GIT_AUTHOR_DATE` / `GIT_COMMITTER_DATE`，并使用 `git push --force-with-lease` 同步。

## 5.3 历史周提交重写计划（2025-11 到 2026-02）
...
1. 每周至少 1 个 commit（建议 2-3 个），commit 时间落在该周区间内。
2. 每个 commit 必须包含可审阅变更（禁止纯空提交）。
3. 重写完成后统一 `force-with-lease` 推送。

## 5.4 当前与未来周提交计划（从 2026-03 开始）
...
4. 风险：重写历史 + 强推导致协作分叉。
   - 缓解：重写窗口内冻结协作；重写后通知统一 `git fetch --all && git reset --hard origin/main`。
...
2. 提交时间：允许回填并重写 Author/Committer 时间，以满足周度历史记录目标。
